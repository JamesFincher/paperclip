...
- Authentication can use either OPENAI_API_KEY or local Codex login. A non-empty OPENAI_API_KEY selects API-key billing; when it is absent, Paperclip relies on the local Codex login session and records codex_local runs as subscription/chatgpt usage.
- In Docker, the published image sets HOME=/paperclip and CODEX_HOME=/paperclip/.codex. Run \`codex login\` or \`codex login --device-auth\` inside the Paperclip container, or copy a file-based auth.json into that persistent CODEX_HOME.
